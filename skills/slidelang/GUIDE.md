...
When the skill is a clean `main` checkout of `aif-projects/slidelang_skill`, its commands fast-forward the repo before loading the command implementation. To refresh the written workflow instructions explicitly, run `npm run self-update && npm run guide` from the skill root.
